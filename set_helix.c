...
Arguments
    ...
    Description
        ...
        set to a helical conformation, i.e. phi in the range -105 to -15 (w/ excep),
        psi in the range -105 to 15 (w/ excep) and omega in the range 180 +/- 5.
        The conformation of a residue is changed if and only if it
        has a non-zero helical weight.  Chi-values for the residues
...
void set_helix(LinusProtein *p, int start, int end, double *s)
{
    ...
    for(i=start;i<end;i++)
    {
        ...
        int index = choice(6,s);
        char ms[3];
        strcpy(ms,HELIX_MS[index]);
        //printf("Meso %s\n",ms);
        ...
        try:
            ...
            phi_atoms[i].sp_angle = uniform(111.0, 113.0)

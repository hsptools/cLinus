...
int make_hbond_list(LinusProtein * p, HbList * hblist,
                     int wmin, int wmax,
                     double hbdist, double hbprobe,
                     double hbtor, double hbenes,
                     double hbenel, double shbdist,
                     double shbtor, double shbene,
                     int use_hbond, int use_schbond, int *size, int *max_acp_size)
{
                    ...
                    if(pos >= minres)
                    {
                        if(use_hbond)
                        {
                            if(*size>0)
                            {
                                ...
                                if(j<6)
                                {
                                    hblist[ctr_hb].acps[ctr_acp].hbene = hbenes;
                                }
                                else
                                {
                                    hblist[ctr_hb].acps[ctr_acp].hbene = hbenel;
                                }
                            }
                            ctr_acp++;
                        }//use_hbond
                        ...
                        if(use_schbond)
                        {
                            ...
                            for(m=0;m<ctr_sc;m++)
                            {
                                ...
                                if(*size>0)
                                {
                                    ...
                                    hblist[ctr_hb].acps[ctr_acp].hbene = shbene;
...
int print_hbond_list(LinusProtein *p, HbList *hblist, int size, char *filename)
{
    ...
    for(i=0;i<size;i++)
    {
        ...
        {
            if(strlen(p->atoms[hblist[i].acps[j].acp].name)!=0)
            {
               fprintf(file, "%d %3s %4s %d %3s %s Distance = %lf to %lf Score = %lf\n",
               p->atoms[hblist[i].donor].resnum +1, p->res_names[p->atoms[hblist[i].donor].resnum] , p->atoms[hblist[i].donor].name,
               p->atoms[hblist[i].acps[j].acp].resnum+1,p->res_names[p->atoms[hblist[i].acps[j].acp].resnum], p->atoms[hblist[i].acps[j].acp].name, //hblist[i].acps[j].acp1->name,
               hblist[i].acps[j].hbdist, hblist[i].acps[j].hbdmax, hblist[i].acps[j].hbene);
            }
